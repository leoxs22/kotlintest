...
suspend fun <T> withSystemProperty(key: String, value: String?, thunk: suspend () -> T): T {
...
suspend fun <T> withSystemProperties(props: Properties, thunk: suspend () -> T): T {
    val pairs = props.toList().map { it.first.toString() to it.second?.toString() }
    return withSystemProperties(pairs, thunk)
}

suspend fun <T> withSystemProperties(props: Map<String, String?>, thunk: suspend () -> T): T {
